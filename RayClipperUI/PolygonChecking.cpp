...
//  RayClipperUI
//
//  Created by Ray Hunter on 22/11/2016.
...
int xyorder( struct coord* p1, struct coord* p2 )
{
...
inline float
isLeft( struct coord P0, struct coord P1, struct coord P2 )
{
...
EventQueue::EventQueue( Polygon P )
{
    ...
    for (int i=0; i < P.size(); i++) {        // init data for edge i
        Eq[2*i]->edge = i;
        Eq[2*i+1]->edge = i;
        Eq[2*i]->eV   = &(P[i]);
        Eq[2*i+1]->eV = &(P[i+1]);
        if (xyorder( &P[i], &P[i+1]) < 0)  { // determine type
            Eq[2*i]->type    = LEFT;
            Eq[2*i+1]->type = RIGHT;
        }
        else {
...
struct _SL_segment {
    int      edge;          // polygon edge i is V[i] to V[i+1]
    struct coord    lP;            // leftmost vertex point
    struct coord    rP;            // rightmost vertex point
...
class SweepLine {
    ...
    AvlTree<struct coord>      Tree;          // balanced binary tree
public:
...
SLseg* SweepLine::add( Event* E )
{
    ...
    struct coord* v1 = (Pn[s->edge]);
    struct coord* v2 = (Pn[s->edge+1]);
